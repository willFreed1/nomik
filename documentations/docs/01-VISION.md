...
NOMIK already tracks **code → code** relationships across TypeScript, JavaScript, Python, Rust, Markdown, SQL, C# migrations, and Python migrations (Django/Alembic). The knowledge graph includes functions, classes, interfaces, imports, call chains, file dependencies, **external API calls**, **database operations**, **environment variables** (USES_ENV edges), **event/message bus** (EMITS/LISTENS_TO edges), **EXPORTS edges**, and **content hashing** (bodyHash for duplicate detection). Health detection: dead code, god objects, god files, duplicate code. **PR Impact Analyzer** (`nomik pr-impact`) for blast-radius analysis. **SRE/Infrastructure tracking**: **Redis** (ioredis, @redis/client, @upstash/redis), **job queues** (Bull/BullMQ/Bee-Queue — PRODUCES_JOB/CONSUMES_JOB edges), **Prometheus metrics** (prom-client — USES_METRIC edges), **Socket.io rooms/namespaces**, **Swagger/OpenAPI** decorator enrichment + setup detection. **OpenTelemetry tracing** (STARTS_SPAN edges, tracer.startSpan/startActiveSpan). **Message broker tracking** (KafkaJS, amqplib, NATS, AWS SQS/SNS — PRODUCES_MESSAGE/CONSUMES_MESSAGE edges with two-pass variable resolution). **Prometheus/Grafana infra config** parsing (alert rules, dashboards, scrape configs). **gRPC/tRPC/GraphQL** procedure tracking (decorator + chain detection). **WebSocket** tracking (ws, @nestjs/websockets, uWebSockets.js). **Docker/K8s config** parsing (Dockerfile, docker-compose, K8s manifests). **CI/CD pipeline** detection (GitHub Actions, GitLab CI). **OpenAPI spec file** parsing (openapi.json/yaml → Route nodes). **Feature flag** tracking (LaunchDarkly, Unleash, Flagsmith, Split.io, GrowthBook, custom env flags). **GraphQL schema** file parsing (.graphql/.gql → Route + Class nodes). **Terraform/IaC** config parsing (.tf → resource, variable, module, data blocks). **Dependency tracking** (package.json, requirements.txt → Module + DEPENDS_ON edges). **Secret/credential detection** (AWS, GitHub, Stripe, Slack, JWT, private keys — SecurityIssue nodes + HAS_SECURITY_ISSUE edges). **.env config parsing** (EnvVar definitions + CONTAINS edges). **Cron job detection** (node-cron, node-schedule, @nestjs/schedule — CronJob + SCHEDULES edges). **CloudFormation/SAM** template parsing (resources, parameters, outputs). **Test coverage correlation** (test file metadata, DEPENDS_ON kind:'test'). **Python runtime tracking** (Redis, Celery, Prometheus, OpenTelemetry, Kafka/RabbitMQ/NATS). **`nomik explain <symbol>`** CLI command for full symbol context (callers, callees, edges, file location). **`nm_explain` MCP tool** for IDE integration. **Cross-service correlation** via `findServiceLinks()` — discovers producer↔consumer pairs sharing topics/queues across files. **`nomik service-links`** CLI command. **`nomik onboard`** — one-command codebase briefing. **`nomik wiki`** — markdown doc generation from graph. **`nomik communities`** — functional cluster detection via Union-Find on CALLS graph with cohesion scoring. **`nomik flows`** — execution flow tracing from entry points (routes, events, queues) through call chains to terminators (DB, API). **`nomik diff <sha1> <sha2>`** — architecture drift detection between scan snapshots. **`nomik badge`** — shields.io health badge generation. God file split: `read.ts` into modular query files (`read-health.ts`, `read-explain.ts`, `read-onboard.ts`, `read-community.ts`, `read-flows.ts`, `read-diff.ts`). **232 tests passing (18 test files)**, 35 CLI commands, **21 MCP tools** + **9 MCP resources** + **6 MCP prompts** + **MCP Sampling**, 8 packages. Stats track ALL 17 node types. Architecture rules engine (9 rules, `.nomik/rules.yaml`), test impact analysis, dependency audit with blast radius, guided migration planner, REST dashboard API (14 endpoints), auto-changelog, role-scoped MCP (dev/architect/security/pm). Supported editors: **Cursor**, **Windsurf**, **Antigravity**, **Claude Desktop**.
